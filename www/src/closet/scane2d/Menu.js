...
import { Galleres } from './Galleres.js';
...
export class Menu  {
  	constructor(glaf) {  		
       ...
       /* this.dCont=new DCont(this.dC);
        ...
        this.sobGalleres=function(s,p,p1){
            trace(s,p,p1)
        }
        ...
        this.array=[]
        this.array[this.array.length]=this.galleres=new Galleres(this, this.sobGalleres);
        this.array[this.array.length]=this.dragPic=new DDragPic(glaf.dCont);
        ...
        this.setConfig=function(config){     
            ...
            this.config=config
            let p=-1;
            for (var i = 0; i < config.three.length; i++) {
                if(config.three[i].keyName=="Galleres"){
                    p=i
                }
            }

            if(p!=-1){
                this.galleres.setArr(config.three[p]);
               /* let arr=config.three[p].array;                
                let o={};
                o.array=[];
